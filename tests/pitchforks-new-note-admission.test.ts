...
check(() => assert.match(source, /onKeyDown=\{event => \{[\s\S]*?event\.key !== 'Tab'/))
check(() => assert.match(source, /querySelectorAll<HTMLButtonElement>\('button:not\(:disabled\)'\)/))
check(() => assert.match(source, /event\.shiftKey[\s\S]*?last\.focus\(\)[\s\S]*?!event\.shiftKey[\s\S]*?first\.focus\(\)/))
