...
assert.match(sources.picker, /import GaborPatch from '\.\/GaborPatch'/, 'the selector uses the production procedural Gabor renderer')
...
assert.equal((sources.picker.match(/<GaborPatch/g) || []).length, 1, 'the Gabor preview uses one production specimen seam')
...
assert.match(sources.picker, /grid grid-cols-1 gap-3 sm:grid-cols-3/)
assert.match(sources.picker, /same 14-row journey\./)
assert.match(sources.picker, /Baseline and retake checks stay Crisp so results remain comparable\. Gabor is practice-only\./)
...
assert.match(sources.training, /practiceMode=\{activePracticeMode\}/)
...
assert.match(sources.vision, /screenPracticeMode=\{screenPracticeMode\}/)
