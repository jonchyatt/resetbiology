...
check(() => assert.match(component, /Math\.abs\(exactCents\(source\.frequency, noteToFreq\(candidate\)\)\) <= MATCH_TOLERANCE_CENTS/))
check(() => assert.match(component, /rangeHeldMsRef\.current >= HOLD_MS/))
