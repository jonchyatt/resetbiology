...
check(() => assert.match(component, /data-testid="pf3-range-not-now"[\s\S]*?className="min-h-12[\s\S]*?NOT NOW — RETURN TO MENU/))
check(() => assert.match(component, /data-testid="pf3-range-comfortable"/))
check(() => assert.match(component, /data-testid="pf3-range-comfortable"[\s\S]*?className="min-h-12/))
check(() => assert.match(component, /data-testid="pf3-range-stop-limit"/))
check(() => assert.match(component, /NOT YET — KEEP \{confirmedLimit \?\? rangeAnchor\} AS MY/))
