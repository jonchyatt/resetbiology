...
(async () => {
  console.log('🚀 Starting StemRegen Playwright scraper...');
  ...
  // Start at StemRegen collections page
  const collectionUrl = 'https://www.stemregen.co/collections/all';
  ...
  console.log(`Found ${productLinks.length} products on StemRegen`);
    ...
    try {
      ...
      const productData = await page.evaluate(() => {
        ...
        document.querySelectorAll('img').forEach(img => {
          ...
          if (src && (src.includes('cdn.shopify.com') || src.includes('stemregen'))) {
        ...
        const supplementMatch = bodyText.match(/Supplement Facts[:\s]*([\s\S]{0,500})/i);
        data.supplementFacts = supplementMatch ? supplementMatch[1].trim() : '';
  ...
  // Save all data
  const filename = 'stemregen-complete-scraped-data.json';
  await fs.writeFile(filename, JSON.stringify(allProducts, null, 2));

  console.log('\n' + '='.repeat(50));
  console.log('🎉 STEMREGEN SCRAPING COMPLETE!');
