...
**FLW STOP-CHECK (real consult `b8r957cvo`, 29.4s):** ✅ STOP + surface to Jon now. "Technically sound… the added deterministic work strengthened the evidence package… ENGRAVING-LOCK stays RED rather than being narrated into completion." Dissent condition (report must not present the stale printed-audit FAILs as additional engraving errors) — SATISFIED: `A4-BLOCKING-REPORT.md` explicitly labels them false + "the engraving is fine." → autonomous loop stopped at the A4 Jon court gate (legitimate per the directive). State externalized (instance + this log + measure-map.json + committed artifacts); compaction-safe.

---

## CORRECTION — staff-offset bug in the verification crops (the disease, inside the verifier)

**Date:** 2026-06-26 (Jon on phone, A4 Court live review). **Jon caught it** by demanding the engraving be shown next to the page ("how am I supposed to check... you show me a single note randomly, no context").

**What happened:** the A4 vision crops (`bari-mNN-tight.png`, from the visual-audit per-measure rects) had a SILENT STAFF OFFSET — for some systems the crop landed on **OLIN (Bass)** instead of **OLIVER (Baritone)**. Confirmed by rendering the full labeled system: m9's earlier crop showed the OLIN staff (bass "shy" sits low, ~Eb2). So the Claude+Argus reads of "m9 shy sits LOW → engraving F3 is wrong" were **reading the wrong voice** — the bass line is naturally low. m5's crop happened to land on OLIVER (correct), which is why it read ~F3.

**Result — RETRACTION:** the "m9 'shy'=F3 is the standout real error" finding is **WITHDRAWN** — it was a staff-offset artifact, not an engraving error. OLIVER's "shy" note looks consistent with our F3. The per-note vision reads in `vision-reads.json` are unreliable (wrong-staff contamination) and are NOT to be cited. This is the EXACT disease PLUMB exists to kill — verification confidently reading the wrong evidence — occurring **inside the verification layer itself**. The deterministic layers (corrections-diff, measure-map, the 12-point printed-manifest corroboration) are unaffected (no vision); only the vision-crop reads were corrupted.

**Fix:** A4 Court rebuilt to show OUR full engraving LINE (measures X–Y, one row) vs the full printed SYSTEM with every voice LABELED (EWART/OLIVER/OLIN) — Jon compares our Baritone engraving to the OLIVER staff. No single floating notes; no auto-staff-targeting to silently drift. Deployed `6a0a041f`.

**Lesson (doctrine):** an auto-derived crop that targets one staff in a multi-staff system MUST be label-verified (does the crop show the intended part's label?) before any pitch is read from it — exactly FLW's #1 risk (silent staff/measure offset). The two-eye vision was never the court (correctly), but a wrong-staff crop makes even corroboration worthless. Ground-truth = the labeled page, shown to Jon. **A2 vision-read findings stand RETRACTED pending Jon's read on the corrected, labeled comparison.**
