...
async function verifyArtifacts() {
  ...
  assert(health.scoreVersion === LEAD_SCORE_VERSION, `expected scoreVersion=${LEAD_SCORE_VERSION}, got ${health.scoreVersion}`);
  assert(Array.isArray(health.wholeNotes) && health.wholeNotes.map((n) => `${n.measure}:${n.pitch}`).join(' ') === EXPECTED_WHOLE_NOTES,
  ...
  assert(Array.isArray(noteMap.notes) && noteMap.notes.length === EXPECTED_LEAD_NOTE_COUNT,
    `expected ${EXPECTED_LEAD_NOTE_COUNT} note-map entries, got ${noteMap.notes?.length}`);
...
async function verifyBrowser() {
  ...
  try {
    ...
    await page.getByText(new RegExp(`Score target\\s+.*Lead\\s+.*${EXPECTED_LEAD_NOTE_COUNT}`, 'i')).first().waitFor({ timeout: 10000 });
    ...
    assert(active.index >= 0 && active.index < EXPECTED_LEAD_NOTE_COUNT, `active index out of range: ${active.index}`);
