...
if (!/isolated Baritone audio/i.test(sync.source || '')) errors.push(`sync source is not audio-derived: ${sync.source || 'missing'}`);
if ((health.sync?.audioConfirmed ?? 0) < 70) errors.push(`audioConfirmed too low: ${health.sync?.audioConfirmed ?? 'missing'}`);
...
console.log(`audio sync: confirmed ${health.sync?.audioConfirmed}; interpolated ${health.sync?.audioInterpolated}; isolated ${health.sync?.isolatedOnsets}`);
