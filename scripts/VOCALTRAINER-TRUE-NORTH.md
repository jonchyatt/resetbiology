...
**Tempo Trainer (pitch-preserving slow-down / speed-up) — *shipped 2026-06-27*.** A global **🐢 Tempo** control by the transport: slider 50–125% + presets (60/75/90/100/110), default 100%. Slow it down to learn, speed it up to test — **pitch never changes**. Built on a **real phase-vocoder (SoundTouch)**, deliberately NOT `AudioBufferSourceNode.playbackRate` (which would chipmunk the pitch) and NOT the browser-native `preservesPitch` throwaway. We render the vocal + music buffers **offline** (before play) to time-scaled buffers, then play them at native rate — so the proven sample-accurate **dichotic lockstep engine is left completely intact**; the song clock and the plunk scheduler simply convert by the speed factor (`songtime = wall × spd`). At 100% the play path is **bit-identical** to before. Tempo is **global, never per-track** (per-track tempo would break sync). *Upgrade path if quality is ever audibly thin: Rubber Band / SoundTouch in an AudioWorklet for real-time, no-re-render stretch — but the offline phase-vocoder is the "amazing from the beginning" build, not a stopgap.*
...
- **TransportOrb** — floating, draggable, always-reachable Play/Stop/Loop (Phase 1 of the layout system). *(shipped 2026-06-27)*
