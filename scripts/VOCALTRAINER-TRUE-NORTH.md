...
**Tempo Trainer (pitch-preserving slow-down / speed-up) — *shipped 2026-06-27*.** A global **🐢 Tempo** control: slider **50–200%** + presets (60/75/90/100/125/150/200), default 100%. Lives in the Mixing Desk **and** mirrored on the orb (⏪ / tap-to-reset / ⏩). Slow it down to learn, speed it up to test — **pitch never changes**. Built on a **real phase-vocoder (SoundTouch)**, deliberately NOT `AudioBufferSourceNode.playbackRate` (which would chipmunk the pitch) and NOT the browser-native `preservesPitch` throwaway. We render the vocal + music buffers **offline** (before play) to time-scaled buffers, then play them at native rate — so the proven sample-accurate **dichotic lockstep engine is left completely intact**; the song clock and the plunk scheduler simply convert by the speed factor (`songtime = wall × spd`). At 100% the play path is **bit-identical** to before. Tempo is **global, never per-track** (per-track tempo would break sync). *Upgrade path if quality is ever audibly thin: Rubber Band / SoundTouch in an AudioWorklet for real-time, no-re-render stretch — but the offline phase-vocoder is the "amazing from the beginning" build, not a stopgap.*
...
- **Tempo Trainer** — pitch-preserving 50–125% slow-down/speed-up (SoundTouch phase-vocoder, offline render, global). *(shipped 2026-06-27)*
- **TransportOrb** — floating, draggable, always-reachable Play/Stop/Loop + tempo (Phase 1 of the layout system). *(shipped 2026-06-27)*
- **WODEN-style orb (the controller).** Collapses to a tiny glowing **solar ball** (copied from WODEN's `WodenFloatingOrb` — layered radial-gradient + orb-outer/mid/core pulse). Gesture model mirrors WODEN: **short-press → transport menu** (Play/Pause/Stop/Loop/gear/time) · **long-press (500ms) → jump/nav menu** (Library/Upload/Score/Tracker/Desk/Top) · **double-tap → Mixing Desk** · **drag → move freely** (threshold-gated so taps still fire; survives rotation). The orb **hides while the desk is open** so it can never block the desk's ✕; the ✕ collapses the desk back to the orb. *(shipped 2026-06-27)*
- **Mixing Desk = a real compact console.** A small bottom-right panel (≤360px, NOT full-screen), launched from the orb. Per-channel strips (Vocals/Music/Mic) use small rotary **knobs** (`MixKnob`): a dial by default, **tap → big spin-knob popover** (drag up/down + −/+) → ✕ collapses back down. Tiny per-strip level meter. Tempo slider + plunk + output + mic-profile live in the same panel. *(shipped 2026-06-27)*
- **Plunk defaults OFF** (opt-in). **Returning to the tab no longer stalls** — the AudioContext resumes + mic re-arms on visibility (phone Chrome suspends background tabs; full-discard reloads remain unavoidable). *(2026-06-27)*
