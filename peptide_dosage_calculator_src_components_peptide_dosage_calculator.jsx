...
/*********************************
 * Brand Color Utilities (scoped)
 *
 * Tailwind classes like bg-primary-600 are referenced by spec.
 * We define lightweight global utilities so you can use the
 * exact class names before adding them to tailwind.config.
 *********************************/
const BrandUtilities = () => (
  <style jsx global>{`
    :root {
      --rb-primary: #3FBFB5;
      --rb-primary-700: #32a79e;
      --rb-primary-600: #3FBFB5; /* same for simplicity */
      --rb-primary-500: #52c9c0;
      --rb-primary-400: #7fd7d1;
      --rb-secondary: #72C247;
      --rb-text-gray: #6B7280;
    }

    .text-primary-400 { color: var(--rb-primary-400) !important; }
    .text-primary-500 { color: var(--rb-primary-500) !important; }
    .text-primary-600 { color: var(--rb-primary-600) !important; }

    .bg-primary-600 { background-color: var(--rb-primary-600) !important; }
    .hover\:bg-primary-700:hover { background-color: var(--rb-primary-700) !important; }

    .border-primary-400\/30 { border-color: color-mix(in oklab, var(--rb-primary-400) 30%, transparent) !important; }
    .ring-primary-400 { --tw-ring-color: var(--rb-primary-400) !important; }

    .text-secondary { color: var(--rb-secondary) !important; }

    /* Subtle frosted card backdrop */
    .rb-card { background: linear-gradient(135deg, rgb(31 41 55 / 0.20), rgb(17 24 39 / 0.30)); }
  `}</style>
);
...
const SyringeVisual: React.FC<{
  ...
  return (
    <div className="relative rb-card rounded-xl p-6 border border-primary-400/30 w-full max-w-sm mx-auto">
...
const ReconstitutionGuide: React.FC<{ peptideAmount: number; volume: number; instructions?: string }>
  = ({ peptideAmount, volume, instructions }) => {
    return (
      <div className="space-y-4 rb-card rounded-xl p-4 border border-primary-400/30">
...
const UnitConverter: React.FC = () => {
  ...
  return (
    <div className="rb-card rounded-xl p-4 border border-primary-400/30 space-y-3">
        ...
        <div className="text-gray-300 text-sm">
          ...
          <div>
            Days remaining: {Number.isFinite(daysRemaining) ? (
              <span className={daysRemaining <= 7 ? "text-amber-300" : "text-secondary font-semibold"}>{daysRemaining} day{daysRemaining === 1 ? "" : "s"}</span>
            ) : "—"}
          </div>
    ...
    <div className="bg-gradient-to-br from-gray-800/90 to-gray-900/90 backdrop-blur-sm rounded-xl p-6 border border-primary-400/30 shadow-2xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          {/* Preset selector */}
          <div className="rb-card rounded-xl p-4 border border-primary-400/30">
          ...
          <div className="rb-card rounded-xl p-4 border border-primary-400/30 space-y-3">
              ...
              <label className="flex-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setInputs((s) => ({ ...s, desiredDose: clamp(parseFloat(e.target.value) || 0, unitMinMax.min, unitMinMax.max) }))}
                    className="flex-1 bg-gray-800/50 border border-gray-600/30 rounded-lg px-3 py-2 text-white placeholder-gray-400 focus:border-primary-400 focus:outline-none"
                  />
                  <button
                    type="button"
            ...
            <input
              ...
              className="w-full accent-[#3FBFB5]"
            />
          ...
          <div className="rb-card rounded-xl p-4 border border-primary-400/30 space-y-3">
            ...
            <div className="flex items-center gap-2 pt-2">
              <input
                ...
                className="h-4 w-4 rounded border-gray-600/30 bg-gray-800/50 text-[#3FBFB5] focus:ring-primary-400"
              />
        ...
        <div className="space-y-4">
          ...
          <div className="rb-card rounded-xl p-4 border border-primary-400/30" role="status" aria-live="polite">
