...
function validateAtlas(json: unknown, image: HTMLImageElement): SpriteAtlas {
  ...
  return { image, frames, frameW: json.frameW, frameH: json.frameH }
}

export function loadSpriteAtlas(jsonUrl: string, imageUrl: string): Promise<AtlasLoadResult> {
  const cacheKey = `${jsonUrl}\u0000${imageUrl}`
  const cached = atlasCache.get(cacheKey)
  if (cached) return cached

  const load = Promise.all([fetchJson(jsonUrl), loadImage(imageUrl)])
    .then(([json, image]) => ({ status: 'ready', atlas: validateAtlas(json, image) }) as const)
