    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-primary-600/20 to-secondary-600/20 backdrop-blur-sm rounded-xl p-6 border border-primary-400/30 shadow-2xl">
        ...
        {completedToday.length > 0 && (
          <div className="mt-4 bg-secondary-600/20 border border-secondary-400/30 rounded-lg p-3">
      ...
      {/* Quick Tips */}
      <div className="bg-blue-600/20 backdrop-blur-sm border border-blue-400/30 rounded-xl p-4">
...
function ExerciseCard({ exercise, isCompleted, onSelect }: ExerciseCardProps) {
  ...
  return (
    <div
      className={`relative bg-gradient-to-br from-gray-800/80 to-gray-900/80 backdrop-blur-sm border rounded-xl p-5 transition-all duration-300 hover:border-primary-400/30 hover:shadow-lg hover:shadow-primary-500/20 hover:scale-[1.02] cursor-pointer group ${
        isCompleted ? 'border-secondary-400/30' : 'border-primary-400/20'
      }`}
      onClick={onSelect}
    >
      {isCompleted && (
        <div className="absolute top-3 right-3 bg-secondary-600/30 text-secondary-300 text-xs px-2 py-1 rounded-full">
          Completed
        </div>
      )}
      ...
      {exercise.focus && exercise.focus.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1">
          {exercise.focus.slice(0, 3).map(f => (
            <span key={f} className="text-xs px-2 py-0.5 bg-gray-700/30 rounded text-gray-400">
              {f}
            </span>
          ))}
        </div>
